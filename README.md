...
```ini
参数                      简写      说明
--listen, -l              -l       SOCKS5代理服务器监听地址 (默认随机端口，监听0.0.0.0)
--http-listen, -H         -H       HTTP代理服务器监听地址 (默认随机端口，监听0.0.0.0)
...
- **监听地址**: 默认监听0.0.0.0，如需安全请指定127.0.0.1（仅本地访问）
...
./ipv6-proxy --auto-detect-ips --verbose

# 只检测IPv4地址
./ipv6-proxy --auto-detect-ipv4 --verbose

# 只检测IPv6地址（适用于IPv6优先场景）
./ipv6-proxy --auto-detect-ipv6 --verbose
...
curl -x http://anyuser%1:anypass@localhost:38080 https://ipinfo.io

# 通过HTTP代理使用索引0的IP（2001:db8::/64）
curl -x http://test%0:dummy@localhost:38080 https://ipinfo.io

# 在Python中使用
import requests
proxies = {
    'http': 'http://user%2:pass@localhost:38080',
    'https': 'http://user%2:pass@localhost:38080'
}
...
curl --socks5 localhost:20808 --socks5-basicauth 'myuser%0:mypass' https://ipinfo.io

# 使用CIDR列表中第2个IP（192.168.1.0/24）
curl --socks5 localhost:20808 --socks5-basicauth 'myuser%2:mypass' https://ipinfo.io
```
...
curl -x http://myuser%1:mypass@localhost:38080 https://ipinfo.io
```
...
proxies = {
    'http': 'http://myuser%0:mypass@localhost:38080',
    'https': 'http://myuser%0:mypass@localhost:38080'
}
...
# 使用curl通过SOCKS5代理访问网站（需查看日志获取实际端口）
curl --socks5 localhost:SOCKS_PORT https://ipinfo.io
```
...
# 使用curl通过HTTP代理访问网站（需查看日志获取实际端口）
curl -x http://localhost:HTTP_PORT https://ipinfo.io
```
